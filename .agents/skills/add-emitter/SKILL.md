...
## Work inside the active experiment

When an experiment exists, read its persisted request, frozen research,
requirements, rubric, references, and current phase before changing
the library:

```bash
verismill status <experiment-dir>
verismill verify <experiment-dir>
```

Register the builder model/action as an `agent-run` receipt, render through
`verismill emit`, and record development selection through `verismill
development`. The experiment owns candidate hashes, pins, manifests, attempts,
and causal history. Do not hand-write build events or treat a demo PDF as the
experiment state.
...
## 2. Enter testable requirements

Add requirements to the active experiment's preparation bundle before it is
frozen. If the experiment is already climbing and the review discovered a new
capability requirement, record that as sourcing/development debt and create a
fresh rubric revision or rerun rather than silently mutating the frozen
instrument. Say **why** the tool was admitted — the tell class it answers, or
the domain it opens. Name one requirement per property a test can check:

```json
[
  {
    "id": "acord.premium-foots",
    "property": "schedule premiums equal exposure / 1000 × rate",
    "failure": "a displayed premium does not recompute from its row"
  }
]
```
...
Generate a candidate through the experiment, then **look at the rendered
...
```bash
verismill agent-run <experiment-dir> --file <builder-receipt.json>
verismill emit <experiment-dir> --class <name> --builder-run <receipt-ref> \
    --explanation <explanation.json> --seed <n> --pins <pins.json>
```

The manifest records class, version, canon, pins, seed, sha256, and the ground
truth of anything `defect=` planted. It is the reproduction recipe, which is
why generated bytes do not belong in the repository. `verismill emit` persists
the exact artifact and manifest as immutable experiment objects and records the
candidate event on the bus.

After visual development review, record `decision=select`. That decision
automatically seals the candidate for blind measurement. Continue immediately
through `climb-round` and its fresh `k=3` panel; blind measurement is part of the
emitter build, not a separate optional request. Do not report the emitter as
complete merely because its capability tests and development inspection pass.

Update the mattermill README usage block, and register only the class's static
