...
def build_rubric_absolute_brief(*, rubric: dict, persona: str,
    ...
    return (
        ...
        "CANONICAL-FACT RULE. Named people, organizations, places, accounts, and "
        "case identifiers may be safe in-world facts. Unless a frozen dimension "
        "explicitly requires external verification, do not treat the inability to "
        "find or recognize a proper noun as evidence of synthesis. Treat those facts "
        "as given and judge whether the document's producing-system anatomy, "
        "relationships, operations, visual construction, and epistemic boundaries "
        "would be credible if the named facts were true. Invented-looking workflow "
        "furniture, impossible identifiers, contradictions, and forensic mismatches "
        "remain valid tells.\n\n"
        "GENERATION-PROVENANCE RULE. This is a measurement of artifact realism, "
        "not a demand that generated bytes conceal their origin. The artifact may "
        "truthfully name a renderer or PDF library in file metadata. Do not treat "
        "that fact alone as a realism defect, and never reward a false producer "
        "claim. For the authenticity field, use genuine when the artifact credibly "
        "functions as the claimed working in-world artifact and synthetic when it "
        "reads as a specimen, template, or prop. Still penalize contradictions "
        "between claimed and actual construction, an implausible production path, "
        "or build/measurement provenance leaking into the visible in-world record.\n\n"
